...
export default function ChicasOrcamentos() {
  ...
  if (loading && orders.length === 0) return <div className="flex items-center justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-rose-500" /></div>;
      ...
      <Card>
        <CardContent className="p-0">
          <div className="grid gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 p-6">
            {paged.map((o) => (
              <div key={o.id} className="border border-rose-200/60 rounded-xl p-5 hover:border-rose-500/30 hover:shadow-md transition-all bg-card flex flex-col space-y-4">
                {/* Header Info */}
                <div className="flex items-start justify-between">
                  ...
                  <StatusBadge 
                    status={o.status === "paid" ? "success" : o.status === "pending" ? "warning" : "primary"} 
                    label={orderStatusLabel(o.status)} 
                  />
                </div>

                {/* Amount */}
                <div className="pb-3 border-b border-rose-200/50">
                  <div className="flex items-baseline justify-between">
                    <span className="text-xs text-muted-foreground font-medium">Valor total do pedido:</span>
                    <span className="text-xl font-bold text-rose-600">{formatCurrency(Number(o.total_amount))}</span>
                  </div>
                </div>
                ...
                <div className="pt-3 flex flex-col gap-2 border-t border-rose-200/50">
                  {/* Select for Editing Status in Card */}
                  <Select value={o.status} onValueChange={(val) => updateStatus(o.id, val)}>
                    <SelectTrigger className="h-8 text-xs w-full border-rose-200 bg-rose-50/50 hover:bg-rose-50 transition-colors">
        ...
        <DialogContent className="max-w-xl max-h-[85vh] overflow-y-auto">
          ...
          {detailOrder && (
            <div className="space-y-6 pt-4">
              <div className="flex items-start justify-between bg-rose-50/50 p-4 rounded-lg border border-rose-200/50">
                <div>
                ...
                <StatusBadge 
                  status={detailOrder.status === "paid" ? "success" : detailOrder.status === "pending" ? "warning" : "primary"} 
                  label={orderStatusLabel(detailOrder.status)} 
                />
              </div>

              <div className="grid grid-cols-2 gap-4 text-sm bg-rose-50/30 p-4 rounded-lg border border-rose-200/50">
                <div>
                ...
                {detailOrder.event_date && (
                  <div>
                    ...
                    <p className="font-medium text-rose-600">{formatDate(detailOrder.event_date)}</p>
                  </div>
                )}
                ...
                {detailOrder.notes && (
                  <div className="col-span-2 mt-2">
                    ...
                    <p className="font-semibold text-foreground bg-rose-50/50 p-2 rounded text-sm italic">{detailOrder.notes}</p>
                  </div>
                )}
              </div>
              ...
              <div className="border-t pt-4 space-y-3">
                <p className="text-sm font-bold uppercase text-muted-foreground tracking-wider flex items-center justify-between">
                  ...
                  <span className="bg-rose-100 text-rose-600 px-2 py-0.5 rounded text-xs">{orderItems[detailOrder.id]?.length || 0} itens</span>
                </p>
                  ...
                  {orderItems[detailOrder.id]?.map((item: any) => (
                    <div key={item.id} className="flex items-center gap-3 p-3 border rounded-lg hover:bg-muted/30 transition-colors">
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-bold text-rose-600 flex items-end justify-end gap-1">
              ...
              <div className="border border-rose-200/50 bg-rose-50/50 p-4 rounded-lg space-y-2">
                ...
                <div className="flex items-center justify-between border-t border-rose-200/50 pt-3 mt-1">
                  <span className="font-black text-lg text-rose-600 uppercase">Valor Final:</span>
                  <span className="font-black text-2xl text-rose-600">{formatCurrency(Number(detailOrder.total_amount))}</span>
                </div>
              </div>
            </div>
          )}
